...
import com.ap.component.TreeComponent;
...
public class Axe extends Tool {
    ...
    public Axe(AtlasAsset atlasAsset, String atlasKey) {
        super("Axe", atlasAsset, atlasKey, AbilityType.Foraging);
    }
    ...
    public void applyItem(WorldObject body, Engine engine, PlayerManager playerManager, World world) {
        ...
        playerManager.getAudioService().playSound(SoundAsset.Axe);
        ...
        if(item.getName().equals(ItemNames.Wood.name())) {
            ...
            playerManager.getInventory().addItem(wood, 1);
        } else if(item instanceof Tree tree) {
            handleTree(engine, playerManager, world, entity, wood);
            ...
            if(crop.getAxeHit() == Constraints.NUMBER_OF_AXE_NEED_TO_DESTROY_GIANT) {
                ...
                for(Item gain : crop.produceItems()) {
                    playerManager.getInventory().addItem(gain, itemAmount);
                }
                Helper.removeEntity(entity, engine, world);
            } else {
    ...
    private void handleTree(Engine engine, PlayerManager playerManager, World world, Entity entity, Item wood) {
        var treeComp = TreeComponent.mapper.get(entity);

        if(!treeComp.hasLeaf()) {
            playerManager.getInventory().addItem(wood, Constraints.STUMP_GIVEN_WOOD);
            Helper.removeEntity(entity, engine, world);
            return;
        }
        ...
        Timer.Task fallTree = new Timer.Task() {
            ...
            public void run() {
                ...
                if(degree[0] * degree[0] >= 85f) {
                    ...
                    playerManager.getInventory().addItem(wood,
