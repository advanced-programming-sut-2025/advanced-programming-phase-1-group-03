...
public class Scythe extends Tool{
    ...
    public Scythe(AtlasAsset atlasAsset, String atlasKey) {
        super("Scythe", atlasAsset, atlasKey, AbilityType.Farming);
    }
    ...
    public void applyItem(WorldObject body, Engine engine, PlayerManager playerManager, World world) {
        ...
        playerManager.getAudioService().playSound(SoundAsset.Scythe);
        ...
        if(item.getName().equals(ItemNames.Grass.name())) {
            ...
            if(new Random().nextInt(10) < Constraints.PROB_OF_GRASS_GIVE_FIBBER) {
                ...
                playerManager.getInventory().addItem(fiber, 1);
            }
        }
        if(item instanceof Plant plant) {
            ...
            if(growable.canProduce()) {
                for(Item gathered : plant.produceItems()) {
                    playerManager.getInventory().addItem(gathered, 1);
                }
